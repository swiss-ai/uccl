...
UcclProxy::UcclProxy(int thread_idx, uintptr_t gpu_buffer_addr,
  ...
  if (thread_idx == 0) {
    ...
    atomic_buffer_is_host_allocated_ = false; // Uses unified memory when we are in Grace hopper (so Alps clariden will use this)
...
void UcclProxy::start(Mode m) {
  ...
  thread_ = std::thread([this]() {
    try {
      ...
      std::fprintf(stderr,
                   "UcclProxy thread %d failed with exception: %s\n",
                   thread_idx_, e.what());
      std::fflush(stderr);
      std::terminate();
    } catch (...) {
      std::fprintf(stderr, "UcclProxy thread %d failed with unknown exception\n",
                   thread_idx_);
      std::fflush(stderr);
      std::terminate();
    }
