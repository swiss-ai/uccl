...
void CxiTransport::init(ProxyCtx& ctx) {
  ...
  // Endpoint address exchange is intentionally left for the proxy bootstrap
  // integration step. Until then this backend must not be selected at runtime.
  std::fprintf(stderr,
               "[CXI] libfabric/CXI endpoint initialized; bootstrap address "
               "exchange is not wired yet\n");
}
