...
echo "[cxi-runner] host=$(hostname -f) node_rank=${node_rank}/${nnodes} master=${master_addr}:${MASTER_PORT}"
echo "[cxi-runner] transport=${UCCL_EP_TRANSPORT} provider=${FI_PROVIDER} gpus=$(python3 -c 'import torch; print(torch.cuda.device_count())')"
...
python3 -c "import torch; import uccl.ep; print('[cxi-runner] import ok torch', torch.__version__, 'gpus', torch.cuda.device_count(), flush=True)"
